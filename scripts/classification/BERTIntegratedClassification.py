...
for patient_subset in tqdm(range(1, 3)):
    for feature_subset in tqdm(range(1,4)):
        for engaged in tqdm(range(0,2)):
          for seed in tqdm(range(0,50,10)):
            ...
            # for seed in range(0,50,14):
                ...
                if feature_subset == 3 and vectorize_text == 2:
                    ...
                    tokens = tokenizer(suhi_df['combined_notes'].tolist(), padding=True, truncation=True, return_tensors='pt')
                ...
                if feature_subset == 3:
                    if vectorize_text == 1:
                        ...
                        combined_notes_vectorized_df = pd.DataFrame(text_embeddings.detach().numpy())
                ...
                for clf_name, (clf, param_grid) in classifiers_and_grids.items():
                    ...
                    test_sensitivity = round(recall_score(y_test, y_pred), 4)
                    test_specificity = round(recall_score(y_test, y_pred, pos_label=0), 4)
